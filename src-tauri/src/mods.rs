    ...
    task::spawn_blocking(move || {
        ...
        for rel in relative_paths {
            ...
            if sanitized.contains("..") {
                return Err("Rejected path with '..'".to_string());
            }
            let abs = base_can.join(&sanitized);
            if !abs.starts_with(&base_can) {
                return Err("Rejected path outside base".to_string());
            }
...
impl ToSlash for &Path {
    fn to_slash(&self) -> Option<String> {
        Some(self.to_string_lossy().replace('\\', "/"))
    }
}
